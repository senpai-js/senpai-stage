import { awayDown, awayUp, common, hoverDown, hoverUp, noop, pointMoveOrderCommon } from "./commonSetupPanel";
...
describe("Sprite pointMoveEvent emitter", () => {
  ...
  test("point with sprite on panel that moves before label added under it and then moves", () => {
  ...
  test("point with sprite on panel that doesn't move before label added under it and then moves", () => {
    const test = pointMoveOrderCommon();
    const { callbacks } = test.feed(t => t
        .addInteractionPoint("ip", "Touch")
  ...
  test("point with sprite on panel that is added on top of label and then moves", () => {
    const test = pointMoveOrderCommon();
    const { callbacks } = test.feed(t => t
        .addLabel("sprite", 0, 0)
  ...
  test("point with sprite on panel that moves before label added under it and then doesn't move", () => {
    const test = pointMoveOrderCommon();
    const { callbacks } = test.feed(t => t
        .addInteractionPoint("ip", "Touch")
        .pointMove("ip", 50, 50)
  ...
  test("point with sprite on panel that doesn't move before label added under it and then doesn't move", () => {
    const test = pointMoveOrderCommon();
    const { callbacks } = test.feed(t => t
        .addInteractionPoint("ip", "Touch")
        .addLabel("sprite", 0, 0)).feed(noop).run();
  ...
  test("point with sprite on panel that is added on top of label and then doesn't move", () => {
    const test = pointMoveOrderCommon();
    const { callbacks } = test.feed(t => t
        .addLabel("sprite", 0, 0)
        .addInteractionPoint("ip", "Touch")).feed(noop).run();
